...
export const AuthProvider = ({ children, orgId }) => {
  const [user, setUser] = useState(null);
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [loading, setLoading] = useState(true);

  // مفاتيح LocalStorage مع orgId
  const getUserKey = () => orgId ? `bero_${orgId}_user` : 'bero_user';
  const getUsersKey = () => orgId ? `bero_${orgId}_system_users` : 'bero_system_users';
  ...
  const login = async (username, password) => {
    try {
      ...
      const usersData = localStorage.getItem(getUsersKey());
      let users = [];
